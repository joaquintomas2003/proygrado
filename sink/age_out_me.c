    ...
    for (i = 0; i < FLOWCACHE_ROWS; i++) {
      semaphore_down(&global_semaphores[i]);
        ...
        for (j = 0; j < BUCKET_SIZE; j++) {
            ...
            last_ts = entry->header.last_update_timestamp;

            if (entry->header.packet_count != 0 && get_time_diff_ns(last_ts) > threshold_ns) {
                ...
                semaphore_down(&ring_buffer_sem_G[ring_index]);
                  ...
                  if (f == 0) {
                    ...
                    //       key[key_index] = entry->header.key[key_index];
                    //   }
                    //   mem_write_atomic(&key, (__mem void *)&r_entry->header.key, sizeof(key));
                    // }

                    // // Copy packet_count
                    // {
                    //   __xrw uint32_t pkt_cnt_buf;
                    //   pkt_cnt_buf = entry->header.packet_count;
                    //   mem_write_atomic(&pkt_cnt_buf, (__mem void *)&r_entry->header.packet_count, sizeof(pkt_cnt_buf));
                    // }

                    // // Copy first_packet_timestamp
                    // {
                    //   __xrw uint64_t ts_buf;
                    //   ts_buf = entry->header.first_packet_timestamp;
                    //   mem_write_atomic(&ts_buf, (__mem void *)&r_entry->header.first_packet_timestamp, sizeof(ts_buf));
                    // }

                    // // Copy last_update_timestamp
                    // {
                    //   __xrw uint64_t ts_buf;
                    //   ts_buf = entry->header.last_update_timestamp;
                    //   mem_write_atomic(&ts_buf, (__mem void *)&r_entry->header.last_update_timestamp, sizeof(ts_buf));
                    // }

                    // // Copy request_meta
                    // {
                    //   __xrw uint32_t request_meta;
                    //   request_meta = entry->header.request_meta;
                    //   mem_write_atomic(&request_meta, (__mem void *)&r_entry->header.request_meta, sizeof(request_meta));
                    // }

                    // // Copy int_metric_info_value.node_count
                    // {
                    //   __xrw uint32_t node_cnt_buf;
                    //   node_cnt_buf = entry->header.node_count;
                    //   mem_write_atomic(&node_cnt_buf,
                    //       (__mem void *)&r_entry->header.node_count,
                    //       sizeof(node_cnt_buf));
                    // }

                    // // Copy per-node data
                    // for (k = 0; k < entry->header.node_count && k < MAX_INT_NODES; k++) {
                    ...
                    // Free the bucket
                    {
                      ...
                      mem_write_atomic(&zero_xrw,
                          (__mem void *)&entry->header.packet_count,
                          sizeof(zero_xrw));
                    }
