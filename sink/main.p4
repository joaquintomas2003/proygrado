...
parser MyParser(packet_in packet,
  ...
  state parse_int_header {
    ...
    hop_ml_val = hdr.int_header.hop_metadata_len;
    if (hop_ml_val != 0) {
      meta.nodes_present = meta.stack_size / hop_ml_val;
    } else {
      meta.nodes_present = 0;
    }

    if (meta.nodes_present > MAX_INT_NODES) {
      meta.nodes_present = MAX_INT_NODES;
    }
...
control MyIngress(inout headers hdr,
  ...
  action populate_requested_metadata() {
    ...
    meta.bitmap.hop_latency = (bit<1>) (hdr.int_header.instruction >> 13) & 0x1;
    meta.bitmap.queue_occupancy = (bit<1>) (hdr.int_header.instruction >> 12) & 0x1;
    meta.bitmap.ingress_timestamp = (bit<1>) (hdr.int_header.instruction >> 11) & 0x1;
    meta.bitmap.egress_timestamp = (bit<1>) (hdr.int_header.instruction >> 10) & 0x1;
    meta.bitmap.level2_interfaces = (bit<1>) (hdr.int_header.instruction >> 9) & 0x1;
    meta.bitmap.egress_interface_tx = (bit<1>) (hdr.int_header.instruction >> 8) & 0x1;
    meta.bitmap.buffer_occupancy = (bit<1>) (hdr.int_header.instruction >> 7) & 0x1;
  }
