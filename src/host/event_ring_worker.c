...
void *event_ring_worker(void *arg) {
    ...
    while (!stop) {
        for (int ring = targ->ring_start; ring <= targ->ring_end; ring++) {
            ring_meta current_ring_meta;
            memset(&current_ring_meta, 0, sizeof(ring_meta));

            if (nfp_cpp_area_read(targ->area_ring_metas[ring], 0, &current_ring_meta, sizeof(ring_meta)) < 0) {
            ...
            for (j = 0; j < BATCH_SIZE; j++) {
                ...
                if (nfp_cpp_area_read(targ->area_rings[ring], offset_event_record, &current_ring_entry, sizeof(event_record)) < 0) {
            ...
            if (j > 0) {
                ...
                if (nfp_cpp_area_write(targ->area_ring_metas[ring], 0, &current_ring_meta, sizeof(ring_meta)) < 0) {
